...
- **`npm test`が通ることは、外部API(TMDB)や本番DB(Supabase)に実際に接続できる
  ことの証明ではない**。テストは`fetch`等の外部依存をモックした単体テストであり、
  検証しているのはコード側のロジック(レスポンスの変換・パラメータの組み立て・
  エラー処理など)のみ。実際の疎通確認は、本番の認証情報を設定した環境で
  手動確認する必要がある(PRの説明にもその旨を明記する)
